...
{
    #$session->writeLog('login',$topic,Dumper($query));

    #my $cgiparams = $query->{'param'};
    #my $origurl = $cgiparams->{'origurl'}[0];
    #my $loginName = $cgiparams->{'username'}[0];
    #my $loginPass = $cgiparams->{'password'}[0];

    my $twiki = shift;
    #$twiki->writeLog('collect_openid_params','chito', Dumper($twiki));
    #my $query = $twiki->{cgiQuery};
    my $query = $twiki->{request};
    #my $cgiparams = $query->Vars;
    my $cgiparams = $query->{'param'};
    #$twiki->writeLog('collect_openid_params','query', Dumper($query));
    ...
    $twiki->writeLog('collect_openid_params','chito', Dumper(\@openid_keys));
    $twiki->writeLog('collect_openid_params','chito', Dumper(\%openid_p));
...
$session->writeLog('login','session', Dumper($o_ref->{params}));
	my %result;
...
#        $session->writeLog('openid_provider_login','query', Dumper($query));
#        $session->writeLog('openid_provider_login','query->param', Dumper($query->param));
#        $session->writeLog('openid_provider_login','query->param(password)', Dumper($query->param('password')));
    ...
    if ( exists $openid_p{mode}) {
        ...
        $session->writeLog('openid_provider_login','loginName', Dumper($loginName));
        #$session->writeLog('openid_provider_login','query', Dumper($query));
		...
		if ( $loginName =~ /$TWiki::cfg{LoginNameFilterIn}/ ) {
			my @openids = TWiki::Users::OpenIDMapping::login2openid( $session,
				$loginName );
			$session->writeLog('openid_provider_login','chito', Dumper(@openids));
			$session->writeLog('openid_provider_login','chito', Dumper($loginName));
			if ( @openids ) {
		...
		$session->writeLog('openid_provider_login','query', Dumper($loginName));
 		$session->writeLog('openid_provider_login','claimed id', Dumper($claimed_id));
...
sub login {
    ...
    #  die Dumper($query->{'param'});

    #$session->writeLog('login','session', Dumper($session->{request}));
    ...
    #    $session->writeLog('login','query', Dumper($query));
        #$session->writeLog('login','query->param', Dumper($query->{param}));
        #$session->writeLog('login','query->param(password)', Dumper($query->{param}->{password}));
    my @openid_keys = @{$o_ref->{keys}};
    my %openid_p = %{$o_ref->{params}};
	...
	if ( defined $selected_op ) {
		...
		undef $loginPass;
$session->writeLog('login','query', Dumper($selected_op));
