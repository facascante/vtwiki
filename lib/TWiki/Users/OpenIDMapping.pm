...
sub login2openid {
	...
	#$session->writeLog('login2openid', 'WebHome-login2openid', Dumper($login));
	my $mapping = $session->{users}{basemapping};
	#$session->writeLog('login2openidchito', 'mapping', Dumper($session));
	if ( exists $mapping->{L2U}{lc($login)}) {
		$session->writeLog('login', 'WebHome-login2openid-1', Dumper($mapping->{L2U}{lc($login)}));
		my $cUID = $mapping->{L2U}{lc($login)};
		return cUID2openid( $session, $cUID );
	}
	if ( exists $mapping->{W2U}{$login}) {
		$session->writeLog('login', 'WebHome-login2openid-2', Dumper($mapping->{W2U}{$login}));
